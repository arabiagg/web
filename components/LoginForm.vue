...
const login = async (e: Event) => {
  ...
  if (error !== null) {
    ElMessage({ message: "Login failed", type: "error", appendTo: ".login-form"})
    return
  }
  emit("onLoginSuccess")
}
...
<template>
  <div class="lg:text-xl">Login</div>
  <ElDivider />

  <ElForm v-if="allowEmailLogin" class="login-form" v-on:submit="login" :model="loginForm" label-width="auto">
    ...
    <ElFormItem>
      <ElButton native-type="submit" type="primary">Login</ElButton>
    </ElFormItem>
    <ElDivider />
  </ElForm>


    <ElButton @click="loginWithDiscord" style="background-color: #5865F2">
      Login with discord
